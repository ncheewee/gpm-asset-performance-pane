...
Static RBAC is included for UAT only. Starter passwords match the role name: `GPM`, `Japan PM`, `Vivocity`, `MBC`, and `ARC/mTower`. Role scoping is enforced in the dashboard and edit-data prototype on the client side, so it is not production security. Only GPM sees Portfolio and Countries. Multi-asset roles such as Japan PM, MBC, and ARC/mTower get a Cluster view. Japan PM mock assets are HPB, MON, HNB, OPB, MBP, MBT, and FJM. GPM can store and test a Deepseek API key in local browser storage.
