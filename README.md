...
Static RBAC is included for UAT only. Starter passwords match the role name: `GPM`, `Japan PM`, `Vivocity`, `MBC`, and `ARC/mTower`. Role scoping is enforced in the dashboard and edit-data prototype on the client side, so it is not production security. Multi-asset roles get a Cluster view. GPM can store a Deepseek API key in local browser storage for AI explanation tests.
